...
# from auth_api.scopes import SettingsScopes
# from oauth2_provider.scopes import get_scopes_backend
from server.settings import OAUTH2_PROVIDER


from oauth2_provider.models import (
    AbstractApplication, AccessToken, Grant, RefreshToken
)
...
class MyAuth2Validator(RequestValidator):
    ...
    @staticmethod
    def _load_application(client_id, request):
        ...
        try:
            ...
            if not request.client.is_usable(request):
                log.debug("Failed body authentication: Application {} is disabled".format(client_id))
                return None
            # return request.client
            return True
        except Application.DoesNotExist:
            log.debug("Failed body authentication: Application {} does not exist".format(client_id))
            return None
    ...
    def client_authentication_required(self, request, *args, **kwargs):
        ...
        return super(MyAuth2Validator, self).client_authentication_required(request, *args, **kwargs)
    ...
    @staticmethod
    def _get_token_from_authentication_server(token, introspection_url, introspection_token):
        ...
        if "active" in content and content["active"] is True:
            ...
            max_caching_time = datetime.now() + timedelta(
                seconds=OAUTH2_PROVIDER.get('RESOURCE_SERVER_TOKEN_CACHING_SECONDS')
            )
    ...
    def validate_bearer_token(self, token, scopes, request):
        ...
        introspection_token = OAUTH2_PROVIDER.get('RESOURCE_SERVER_AUTH_TOKEN')
    ...
    def validate_scopes(self, client_id, scopes, client, request, *args, **kwargs):
        ...
        available_scopes = Application.objects.get(client_id=client_id).scopes.split()
        valid = set(scopes).issubset(set(available_scopes))
        return valid

    def get_default_scopes(self, client_id, request, *args, **kwargs):
        # default_scopes = get_scopes_backend().get_default_scopes(application=request.client, request=request)
        # default_scopes = ['read', 'write']
        default_scopes = Application.objects.get(client_id=client_id).scopes.split()
    ...
    def save_authorization_code(self, client_id, code, request, *args, **kwargs):
        expires = timezone.now() + timedelta(
            seconds=OAUTH2_PROVIDER.get('AUTHORIZATION_CODE_EXPIRE_SECONDS'))
    ...
    def rotate_refresh_token(self, request):
        return OAUTH2_PROVIDER.get('ROTATE_REFRESH_TOKEN')
